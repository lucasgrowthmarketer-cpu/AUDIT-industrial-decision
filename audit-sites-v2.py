...
POIDS = {
    "https": 7,
    "rendu_serveur": 10,
    "catalogue_indexable": 8,
    "profondeur_catalogue": 12,   # volume reel de pages indexees
    "sitemap": 5,
    "robots_txt": 4,
    "crawlers_ia": 6,
    "llms_txt": 6,
    "donnees_structurees": 10,    # exige un schema metier
    "title_ok": 6,
    "description_ok": 5,
    "h1_unique": 5,
    "viewport": 4,
...
# Schemas generes automatiquement par la plupart des CMS : ne comptent pas seuls
SCHEMAS_GENERIQUES = {
    "website", "webpage", "breadcrumblist", "collectionpage", "itemlist",
    "searchaction", "sitenavigationelement", "imageobject", "person",
}
...
def audit(nom, url, type_="", pays=""):
    ...
    metier = [t for t in types if t.lower() not in SCHEMAS_GENERIQUES]
    d["schemas_metier"] = sorted(metier)
    # Valide si : 3 schemas dont au moins un metier, OU 2 schemas metier minimum.
    # Evite de valider les sites qui n'ont que WebSite + WebPage + BreadcrumbList,
    # generes automatiquement par la plupart des CMS.
    c["donnees_structurees"] = (len(types) >= 3 and len(metier) >= 1) or len(metier) >= 2
    ...
    # ── Profondeur du catalogue (paliers) ──
    n_urls = d.get("sitemap_urls", 0)
    if n_urls >= 100:
        prof = 1.0
    elif n_urls >= 20:
        prof = 0.5
        ...
        prof = 0.0
    c["profondeur_catalogue"] = prof >= 0.5
    d["profondeur_ratio"] = prof

    # ── Score ──
    score = sum(
        POIDS[k] * (d["profondeur_ratio"] if k == "profondeur_catalogue" else 1)
